...
> **Note:** Before using the Auth class, you must [specify a session driver](/docs/laravel/session/config).
...
It is common to limit access to certain routes only to logged in users. In Laravel this is accomplished using the [auth filter](/docs/laravel/routing#filters). If the user is logged in, the request will proceed as normal; however, if the user is not logged in, they will be redirected to the "login" [named route](/docs/laravel/routing#named-routes).
